...
import PartConfigView from './components/PartConfigView';
import MaterialConfigView from './components/MaterialConfigView';
...
const generateProjectNumber = () => {
  ...
  return `V2-${year}${month}${day}-${random}`;
};
...
const App: React.FC = () => {
  ...
  const renderToolStep = () => {
    switch (step) {
      ...
      case 3: return <MachineConfigView state={state} onUpdate={updateState} onNext={() => setStep(4)} onPrev={() => setStep(2)} />;
      case 4: return <CycleTimeView state={state} results={results} onUpdate={updateState} onNext={() => setStep(5)} onPrev={() => setStep(3)} />;
  ...
  const isProjectActive = step >= 1 && step <= 5;
  ...
  return (
    <div className={`min-h-screen flex flex-row font-display transition-colors duration-200 ${isDarkMode ? 'dark bg-background-dark text-text-dark' : 'bg-background-light text-text-light'}`}>
      {currentView === 'estimating' && (
        <aside className="w-72 flex-shrink-0 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-surface-dark transition-colors duration-200 shadow-xl z-30">
          <div className="flex flex-col h-full">
            {/* Sidebar Header */}
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col gap-1">
               <div className="flex items-center gap-3">
                 <div className="bg-primary size-8 rounded-lg flex items-center justify-center text-white font-black text-sm">V2</div>
                 <h1 className="text-slate-900 dark:text-white text-lg font-bold leading-normal tracking-tight">V2Cost.</h1>
               </div>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
              ...
              <div className="mb-4">
                ...
                <NavItem 
                  ...
                  onClick={() => { if(!isProjectActive) setStep(1); }}
                  hasChildren={isProjectActive}
                />
                
                {/* Project Model Tree Structure - Conditional Rendering */}
                {isProjectActive && (
                  <div className="ml-4 pl-3 border-l-2 border-slate-100 dark:border-slate-800 space-y-1 mt-2 mb-4 animate-fadeIn">
                    ...
                    <TreeItem label="Summary View" icon="pie_chart" active={step === 5} onClick={() => setStep(5)} />
                  </div>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 mt-4">
                ...
                <NavItem label="Machine Database" icon="settings_suggest" active={false} onClick={() => {}} />
                <NavItem label="Cost Summary" icon="payments" active={step === 5} onClick={() => setStep(5)} />
              </div>
            </nav>

            {/* Profile Section */}
            <div className="p-4 border-t border-slate-100 dark:border-slate-800">
               <div className="flex items-center gap-3 px-3 py-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl mb-3">
                <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-sm font-bold text-slate-500 ring-2 ring-white dark:ring-slate-700 shadow-sm overflow-hidden">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 dark:text-slate-500 truncate">alex@company.com</p>
                </div>
              </div>
              <button 
                onClick={() => setCurrentView('landing')}
                className="flex items-center gap-3 px-3 py-2 w-full text-left rounded-lg hover:bg-red-50 dark:hover:bg-red-900/10 text-slate-500 hover:text-red-600 transition-colors font-medium text-sm"
              >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="flex items-center justify-between whitespace-nowrap border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-background-dark px-8 py-3 shrink-0 z-20 shadow-sm">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-4 text-slate-900 dark:text-white">
              ...
              <h2 className="text-lg font-bold leading-tight tracking-tight">
                {step === 6 ? 'Part Library' : (isProjectActive ? `Estimating: ${state.projectName}` : 'V2Cost.')}
              </h2>
            </div>
          </div>
          <div className="flex flex-1 justify-end gap-6 items-center">
            ...
            <button className="text-slate-500 hover:text-primary transition-colors" onClick={handleToggleDarkMode}>
        ...
        <main className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark p-6 lg:p-10">
          {currentView === 'landing' ? (
            <LandingView onStart={handleStartEstimation} />
          ) : (
...
const NavItem: React.FC<{ label: string, icon: string, active: boolean, onClick: () => void, hasChildren?: boolean }> = ({ label, icon, active, onClick, hasChildren }) => (
  <a 
    onClick={onClick}
    className={`flex items-center justify-between px-3 py-2.5 rounded-lg transition-all cursor-pointer group ${active ? 'bg-primary/10 text-primary' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
  >
    <div className="flex items-center gap-3">
      ...
      <p className={`text-sm font-semibold tracking-tight ${active ? 'text-primary font-bold' : ''}`}>{label}</p>
    </div>
    {hasChildren !== undefined && (
      <span className={`material-symbols-outlined text-sm transition-transform duration-200 ${active ? 'rotate-180 text-primary' : 'opacity-50'}`}>
        expand_more
      </span>
    )}
...
const TreeItem: React.FC<{ label: string, icon: string, active: boolean, onClick: () => void }> = ({ label, icon, active, onClick }) => (
  <a 
    onClick={onClick}
    className={`flex items-center gap-2 px-3 py-2.5 rounded-lg transition-all cursor-pointer group relative ${active ? 'bg-primary/10 text-primary' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
  >
    <span className={`material-symbols-outlined text-[18px] ${active ? 'text-primary' : 'text-slate-400 group-hover:text-primary'}`}>{icon}</span>
    <p className={`text-[13px] font-medium leading-tight ${active ? 'font-bold' : ''}`}>{label}</p>
